...
# 项目根目录和测试用例目录
project_root = "."
testcases_root = os.path.join(project_root, "test", "python", "testcases", "lexer-public")
...
def run_tests():
    ...
    for category in ['A', 'B', 'C']:
        ...
        for testcase in sorted(os.listdir(category_path), key=lambda x: int(''.join(filter(str.isdigit, x)))):
            ...
            with open(os.path.join(project_root, "lexer.txt"), 'r') as lexer_output:
                with open(ans_path, 'r') as expected_output:
                    lexer_lines = lexer_output.readlines()
                    expected_lines = expected_output.readlines()

                    # 4. 比较两个文件的内容
                    if lexer_lines == expected_lines:
                        passed_tests += 1
                        ...
                        for i, (lexer_line, expected_line) in enumerate(zip(lexer_lines, expected_lines)):
                            if lexer_line != expected_line:
                                print(f"💡 We expected \"{expected_line.strip()}\" but we got \"{lexer_line.strip()}\" at line {i + 1}")
