...
# 项目根目录和测试用例目录
project_root = "."
testcases_root = os.path.join(project_root, "test", "python", "testcases", "semantic-check-public")
...
def run_tests():
    ...
    for category in ['A', 'B', 'C']:
        ...
        for testcase in sorted(os.listdir(category_path), key=lambda x: int(''.join(filter(str.isdigit, x)))):
            ...
            # 3. 读取 symbol.txt 和 ans.txt
            with open(os.path.join(project_root, "symbol.txt"), 'r') as symbol_output:
                with open(ans_path, 'r') as expected_output:
                    symbol_lines = symbol_output.readlines()
                    expected_lines = expected_output.readlines()

                    # 4. 比较两个文件的内容
                    if symbol_lines == expected_lines:
                        passed_tests += 1
                        ...
                        for i, (symbol_line, expected_line) in enumerate(zip(symbol_lines, expected_lines)):
                            if symbol_line != expected_line:
                                print(f" We expected \"{expected_line.strip()}\" but we got \"{symbol_line.strip()}\" at line {i + 1}")
#                                 exit(1)
